> Let $\otimes$ be an associative binary operator, and let $a$ be an attribute
> maintained in each node of a red-black tree. Suppose that we want to include
> in each node $x$ an additional attribute $f$ such that $x.f = x_1.a \otimes
> x_2.a \otimes \cdots \otimes x_m.a$, where $x_1, x_2, \ldots, x_m$ is the
> inorder listing of nodes in the subtree rooted at $x$. Show how to update the
> $f$ attributes in $\O(1)$ times after a rotation. Modify your argument
