> The procedures `OS-SELECT` and `OS-RANK` use the `size` attribute of a node
> only to compute a rank. Suppose that you store in each node its rank in the
> subtree of which it is the root instead of the `size` attribute. Show how to
> maintain this information during insertion and deletion. (Remember that these
> two operations can cause rotations.)
